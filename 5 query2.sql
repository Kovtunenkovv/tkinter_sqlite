...
select p.zip, p.hostname, p.eventid ,p.firstoccurrence,p.enddate,((Cast((JulianDay((p.enddate)) - JulianDay(p.firstoccurrence)) * 24 * 60 * 60 As Integer))) as delta, h.eventid, h.firstoccurrence,h.enddate,((Cast((JulianDay((h.enddate)) - JulianDay(h.firstoccurrence)) * 24 * 60 * 60 As Integer))) as delta1,du from p
